...
//
//  Modified by Kateryna Streltsova
...
struct Random {
    ...
    init(seed: UInt32, sequence: UInt32) {
        ...
        _ = randomUInt32()
        print("BEFORE: seed \(seed) state: \(state)")
        state = state &+ UInt64(seed)
        _ = randomUInt32()
        print("AFTER: seed \(seed) state: \(state)")
    }
    ...
    mutating func randomUInt32() -> UInt32 {
        ...
        print("randomUInt32: state: \(state) return: \((xorshifted >> rot) | (xorshifted << UInt32(-Int(rot) & 31)))")
